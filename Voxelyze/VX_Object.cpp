/*******************************************************************************
Copyright (c) 2010, Atsushi Masumori
...
// This software is developed based on Voxelyze (https://svn.code.sf.net/p/voxcad/code/VoxCad/Voxelyze) developed by following author with following license.
...
void CVX_Object::ClearMatterFAV(void) //clears out everything...
{
    DrawMode = DisplayMode::MatColor;
    Lattice.ClearLattice();
    Voxel.ClearVoxel();
...
void CVX_Object::LoadDefaultPalette(void)
{
    if (!LoadVXPFile("Default.vxp")){ //try to load the default palette from default VXC object
        ...
        Palette[1].SetColor(1, 0, 0);
        Palette[1].SetElasticMod(1000000.0);
        Palette[1].SetDensity(1.0);
    }
}

void CVX_Object::LoadPaletteFAV(void)
{
...
void CVX_Object::ClearPalette()
{
     ...
     Voxels.clear();
     VecSize = (int)Voxels.size();
     if (VecSize != 0)
         Voxels.erase(Voxels.begin(), Voxels.begin()+VecSize); //erase the Voxels array

     CVXC_VoxelInfo Eraser = CVXC_VoxelInfo("Erase", 0, NULL, NULL, NULL, NULL);
     Voxels.push_back(Eraser);

     VecSize = (int)Geometries.size();
     if (VecSize != 0)
         Geometries.erase(Geometries.begin(), Geometries.begin()+VecSize); //erase the Geometries array
     CVXC_Geometry tmpGeo(0,0);
     Geometries.push_back(tmpGeo);

     VecSize = (int)Materials.size();
     if (VecSize != 0)
         Materials.erase(Materials.begin(), Materials.begin()+VecSize); //erase the materials array
     CVXC_MaterialInfo tmpMat(0);
     Materials.push_back(tmpMat);
}

/*! If the associated structure of this voxel object has not been cleared, there will be material indices left in the structure that do not correspond to existing materials. This situation is to be avoided, as other function calls may produce errors in this situation.
*/
void CVX_Object::ClearPaletteFAV()
{
    int VecSize = (int)Palette.size();
    if (VecSize != 0)
        Palette.erase(Palette.begin(), Palette.begin()+VecSize); //erase the materials array
...
void CVX_Object::AddVoxInfo(std::string Name, int id, double r, double g, double b, double a, std::string* RetMessage)
{
    CVXC_VoxelInfo tmp;
    tmp.SetName(Name);
    tmp.SetId(id);
    tmp.SetMatType(SINGLE);
    tmp.SetColor(r,g,b);
    CVXC_Geometry tmpGeo(Geometries.size());
    CVXC_MaterialInfo tmpMat(Materials.size());
    Materials.push_back(tmpMat);
    Geometries.push_back(tmpGeo);

    tmp.SetGeometryId(Geometries.size());

    if(id != 0){
        tmp.AddMatInfo(Materials.size()-1, 1.0);
    }
...
void CVX_Object::SaveVXFFile(std::string filename, int Compression)
{
    CXML_Rip XML;
    WriteVXF(&XML, Compression);
    XML.SaveFile(filename);
}
...
bool CVX_Object::LoadVXFFile(std::string filename)
{
    CXML_Rip XML;
    if (!XML.LoadFile(filename)) return false;
    ReadVXF(&XML);
...
void CVX_Object::SaveFAVFile(std::string filename, int Compression)
{
    CXML_Rip XML;
    WriteFAV(&XML, Compression);
    XML.SaveFile(filename);
}

bool CVX_Object::LoadFAVFile(std::string filename)
{
    CXML_Rip XML;
    if (!XML.LoadFile(filename)) return false;
    ReadFAV(&XML);
...
void CVX_Object::WriteVXF(CXML_Rip* pXML, int Compression, std::string* RetMessage)
{
    pXML->DownLevel("VXF");
    pXML->SetElAttribute("version", "0.1");
    WriteFAV(pXML, Compression);
    WritePaletteXML(pXML, Compression);
    pXML->UpLevel();
}

void CVX_Object::WriteFAV(CXML_Rip* pXML, int Compression, std::string* RetMessage)
{

    pXML->DownLevel("fav");
    pXML->SetElAttribute("version", "1.0");

    WritePaletteFAV(pXML, Compression);
    for(int i=0; i<Voxels.size(); i++){
        if(Voxels[i].GetId() != 0){
            pXML->DownLevel("voxel");
            Voxels[i].WriteFAV(pXML, Compression);
            pXML->UpLevel();
        }
    }

    pXML->DownLevel("object");
    pXML->SetElAttribute("id", "1");
    pXML->SetElAttribute("name", "test fav");

    pXML->DownLevel("grid");

    pXML->DownLevel("dimension");
    pXML->Element("x", Structure.GetVXDim());
    pXML->Element("y", Structure.GetVYDim());
    pXML->Element("z", Structure.GetVZDim());
    pXML->UpLevel();

    pXML->DownLevel("origin");
    pXML->Element("x", Structure.GetOriginX());
    pXML->Element("y", Structure.GetOriginY());
    pXML->Element("z", Structure.GetOriginZ());
    pXML->UpLevel();

    pXML->DownLevel("unit");
    pXML->Element("x", Lattice.GetXDimAdj());
    pXML->Element("y", Lattice.GetYDimAdj());
    pXML->Element("z", Lattice.GetZDimAdj());
    pXML->UpLevel();

    pXML->UpLevel();

    Structure.WriteFAV(pXML, Compression, RetMessage);
...
void CVX_Object::WritePaletteFAV(CXML_Rip* pXML, int Compression)
{
    pXML->DownLevel("palette");

    for (int i=0; i<(int)Geometries.size(); i++){
        if(Geometries[i].GetId() != 0){
            pXML->DownLevel("geometry");
            pXML->SetElAttribute("id", Geometries[i].GetId());
            pXML->SetElAttribute("name", Geometries[i].GetName());
            Geometries[i].WriteFAV(pXML, Compression);
            pXML->UpLevel();
        }
    }

    for (int i=0; i<(int)Materials.size(); i++){
        if(Materials[i].GetId() != 0){
            pXML->DownLevel("material");
            pXML->SetElAttribute("id", Materials[i].GetId());
            pXML->SetElAttribute("name", Materials[i].GetName());
            Materials[i].WriteFAV(pXML, Compression);
            pXML->UpLevel();
        }
    }
    pXML->UpLevel();
...
void CVX_Object::ReadVXF(CXML_Rip* pXML, bool OnlyPal, std::string* RetMessage)
{
    pXML->FindElement("fav");
    ReadFAV(pXML);
    ClearPaletteFAV();
    ReadPaletteVXF(pXML);
}

void CVX_Object::ReadFAV(CXML_Rip* pXML, bool OnlyPal, std::string* RetMessage)
{
    ClearMatterFAV();
    LoadDefaultPalette();
    ...
    if (pXML->FindElement("metadata")){
        pXML->FindLoadElement("title", &Title);
        pXML->FindLoadElement("author", &Author);
        pXML->FindLoadElement("license", &License);
        pXML->UpLevel();
    }

    if (pXML->FindElement("palette")){
        ReadPaletteFAV(pXML, Version);
        pXML->UpLevel(); //Material
    }else{
//        LoadDefaultPalette();
    }

    while (pXML->FindElement("voxel")){
        CVXC_VoxelInfo tmpVox;
        tmpVox.ReadFAV(pXML, Version);
        tmpVox.SetMatId(1);//set default material
        Voxels.push_back(tmpVox);
    }

    if (!OnlyPal && pXML->FindElement("object")){
        if(!OnlyPal && pXML->FindElement("grid")){
            Lattice.ReadFAV(pXML);
            pXML->UpLevel();
        }

        Structure.ReadFAV(pXML);
        pXML->UpLevel();
        pXML->UpLevel();
    }
    ...
    if (!OnlyPal && pXML->FindElement("Voxel")){
        ...
        Voxel.SetVoxName(VS_BOX);
        Voxel.SetXSqueeze(1.0);
        Voxel.SetYSqueeze(1.0);
        Voxel.SetZSqueeze(1.0);
    }
    pXML->UpLevel();

//    Structure.SetFDMLinkMap();
    if(Structure.hasColor()) DrawMode = DisplayMode::FullColor;
}
...
void CVX_Object::ReadPaletteVXF(CXML_Rip* pXML, std::string Version)
{
    int PrevPalSize = GetNumMaterials(); //store how many materials were in there
...
void CVX_Object::ReadPaletteFAV(CXML_Rip* pXML, std::string Version)
{
//    int PrevPalSize = GetNumMaterials(); //store how many materials were in there

    while (pXML->FindElement("geometry")){
        CVXC_Geometry tmpGeo;
        tmpGeo.ReadFAV(pXML, Version);
        Geometries.push_back(tmpGeo);
        pXML->UpLevel();
    }

    while (pXML->FindElement("material")){
        CVXC_MaterialInfo tmpMat;
        tmpMat.ReadFAV(pXML, Version);
        Materials.push_back(tmpMat);
    }
}
...
int CVX_Object::GetNumTri(){// by Atsushi Masumori
    if(fileio.is_loaded){
        return fileio.meshEntries[0]->mesh->number_of_faces;
    }
    return 0;
}

int CVX_Object::GetMeshScaleX(){// by Atsushi Masumori
    if(fileio.is_loaded){
        return fileio.meshEntries[0]->mesh->bounding_box.length[0];
    }
    return 0;
}

int CVX_Object::GetMeshScaleY(){// by Atsushi Masumori
    if(fileio.is_loaded){
        return fileio.meshEntries[0]->mesh->bounding_box.length[1];
    }
    return 0;
}

int CVX_Object::GetMeshScaleZ(){// by Atsushi Masumori
    if(fileio.is_loaded){
        return fileio.meshEntries[0]->mesh->bounding_box.length[2];
    }
    return 0;
}
...
int CVX_Object::GetMatIndex(int StructValue) //get the final material to display
{
    for(int i=0; i<GetNumVoxels(); i++){
        if(Voxels[i].GetId() == StructValue){
            return Voxels[i].GetMatId();
        }
    }
...
int CVX_Object::GetLeafMatIndex(int StructIndex, bool*pVisible) //get the final material to display
{
    ...
    int LastMatIndex = GetMatIndex(Structure.GetData(StructIndex));
...
*/
bool CVX_Object::SetMat(int StructIndex, int VoxIndex) //sets the material index at this location based on the index (advanced users)
{
    if (VoxIndex >=0 && VoxIndex < GetNumVoxels() && StructIndex >=0 && StructIndex < Structure.GetArraySize()){
        Structure.SetData(StructIndex, VoxIndex);
...
//        strm << "Size: (" << std::setprecision(3) << WS.x << "mm," << WS.y << "mm," << WS.z << "mm)\n"; (atsmsmr)
...
void CVX_Object::GetMeshInfoStr(std::string* pString)
{
    if (pString){
        std::stringstream strm;
        Vec3D<> WS = GetWorkSpace();
        strm << "Number of triangles: " << GetNumTri() << "\n";
        strm << "Size: (" << GetMeshScaleX() << "mm," << GetMeshScaleY() << "mm," << GetMeshScaleZ() << "mm)\n";
//        strm << "Surface Area: " << GetSurfaceArea()*1e6 << "mm^2\n";
        *pString = strm.str();
    }
...
CVXC_MaterialInfo* CVX_Object::GetMaterialInfo(int id){
        for(int i=0; i<Materials.size(); i++){
            if(id == Materials[i].GetId()) {
                return &Materials[i];
            }
        }
        return &Materials[0];
...
CVXC_Geometry* CVX_Object::GetGeometryInfo(int id){

        for(int i=0; i<Geometries.size(); i++){
            if(id == Geometries[i].GetId()) {
                return &Geometries[i];
            }
        }
        return &Geometries[0];
...
/*! Read all information from a pre-initialized and populated XML tree. The current level of the stack should point to a
 *  element.
...
void CVXC_Lattice::ReadFAV(CXML_Rip* pXML)
{
    // fav file format has no following variables
    Lattice_Dim = 1.0;
    X_Line_Offset = 0.0;
    Y_Line_Offset = 0.0;
    X_Layer_Offset = 0.0;
    ...
    if(pXML->FindElement("unit")){
        if (!pXML->FindLoadElement("x", &X_Dim_Adj)) X_Dim_Adj = 1.0;
        if (!pXML->FindLoadElement("y", &Y_Dim_Adj)) Y_Dim_Adj = 1.0;
        if (!pXML->FindLoadElement("z", &Z_Dim_Adj)) Z_Dim_Adj = 1.0;
        pXML->UpLevel();
    }
}

void CVXC_Lattice::WriteFAV(CXML_Rip* pXML)
{
    pXML->DownLevel("Lattice");
        pXML->Element("Lattice_Dim", Lattice_Dim);
        pXML->Element("X_Dim_Adj", X_Dim_Adj);
...
//void CVXC_Material::SetMaterial(std::string matname){
...
//        // there is no template for this material
//        break;
//    }
//}

//void CVXC_Material::ReadXML_FAV(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
//{
//    DStrain.clear();
//    DStress.clear();

//    if(!pXML->FindLoadElement("material_name", &Material_Name)){
...
void CVXC_Structure::IniData(int Size)
{
    ...
    if (Size > 0){
        pData = new char[Size];
        pColor_r = new int[Size]();
        pColor_g = new int[Size]();
        pColor_b = new int[Size]();
        m_SizeOfArray = Size;
        DataInit = true;
        Neighbors = 0;
        ColorMode = "none";
...
void CVXC_Structure::IniData(int Size, int neighbors)
{
    DeleteData();
    if (Size > 0){
        pData = new char[Size];
        ...
        DataInit = true;
        if(neighbors != 0){
            LinkMap = std::vector<std::vector<int>>(Size, std::vector<int>(neighbors,0));
        }
    }
}
...
bool CVXC_Structure::WriteXML(CXML_Rip* pXML, int Compression, std::string* RetMessage)
{
    ...
    for (int i=0; i<Z_Voxels; i++){
        ...
        pXML->Element("Layer", WriteData, false);
    }
    pXML->UpLevel();
    pXML->UpLevel();
    return true;
...
bool CVXC_Structure::WriteFAV(CXML_Rip* pXML, int Compression, std::string* RetMessage)
{
    Compression = CP_ASCIIREADABLE; // this version only for ascii readable
    int Bit_Per_Voxel = 8;
    std::string Compress;
    switch (Compression){
        case CP_RAWDATA: Compress = "RAW_DATA"; break;
        case CP_ASCIIREADABLE: Compress = "none"; break;
        case CP_QTZLIB: Compress = "QT_ZLIB"; break;
        case CP_BASE64: Compress = "BASE64"; break;
        case CP_ZLIB: Compress = "ZLIB"; break;
    ...
    // writing voxel map
    pXML->DownLevel("voxel_map");
    pXML->SetElAttribute("compression", Compress);
    pXML->SetElAttribute("bit_per_voxel", Bit_Per_Voxel);
    ...
    for (int i=0; i<Z_Voxels; i++){
        ...
        RawData.clear();
        for (int j=0; j<X_Voxels*Y_Voxels; j++){
           int n = (int)GetData(i*X_Voxels*Y_Voxels + j);
           char buff[2];
           sprintf( buff, "%02x",  n);
            RawData.push_back(buff[0]);
            RawData.push_back(buff[1]);
        } //convert to QByteArray to use qcompress

        pXML->Element("layer", RawData, true);
    }
    pXML->UpLevel();
    ...
    // writing color map
    if(ColorMode == "RGB"){
        pXML->DownLevel("color_map");
        pXML->SetElAttribute("color_mode", ColorMode);

        std::string RawData_R;
        std::string RawData_G;
        std::string RawData_B;
        ...
        for (int i=0; i<Z_Voxels; i++){
            ...
            for(int j=0; j<X_Voxels*Y_Voxels; j++){
                ...
                if(n != 0){
                    ...
                    char buff[2];
                    sprintf(buff, "%02x",GetColorR(i*X_Voxels*Y_Voxels + j));
                    WriteData.push_back(buff[0]);
                    WriteData.push_back(buff[1]);

                    sprintf(buff, "%02x",GetColorG(i*X_Voxels*Y_Voxels + j));
                    WriteData.push_back(buff[0]);
                    WriteData.push_back(buff[1]);

                    sprintf(buff, "%02x",GetColorB(i*X_Voxels*Y_Voxels + j));
                    WriteData.push_back(buff[0]);
                    WriteData.push_back(buff[1]);
    ...
    if(Neighbors > 0){
        ...
        pXML->DownLevel("link_map");
        pXML->SetElAttribute("compression", Compress);
        pXML->SetElAttribute("neighbors", Neighbors);
        ...
        for (int i=0; i<Z_Voxels; i++){
            for(int j=0; j<X_Voxels*Y_Voxels; j++){
                ...
                if(n != 0){
                    char buff[2];
                    for (int current_neighbor=0; current_neighbor<Neighbors; current_neighbor++){
                        sprintf(buff, "%02x",LinkMap[i*X_Voxels*Y_Voxels + j][current_neighbor]);
                        WriteData.push_back(buff[0]);
                        WriteData.push_back(buff[1]);
                    }
                }
...
void CVXC_Structure::SetFDMLinkMap()
{
    for(int z=0; z<Z_Voxels; z++){
        for(int y=0; y<Y_Voxels; y++){
            for(int x=0; x<X_Voxels; x++){
                ...
                if(pData[index] != 0){
                    if(z-1 > 0 && pData[GetIndex(x,y,z-1)] != 0) LinkMap[index][0] = 255;
                    if(x-1 > 0 && pData[GetIndex(x-1,y,z)] != 0) LinkMap[index][1] = 100;
                    if(y-1 > 0 && pData[GetIndex(x,y-1,z)] != 0) LinkMap[index][2] = 200;
                    if(y+1 < Y_Voxels && pData[GetIndex(x,y+1,z)] != 0) LinkMap[index][3] = 200;
                    if(x+1 < X_Voxels && pData[GetIndex(x+1,y,z)] != 0) LinkMap[index][4] = 100;
                    if(z+1 < Z_Voxels && pData[GetIndex(x,y,z+1)] != 0) LinkMap[index][5] = 255;
                }
            }
        }
    }
...
bool CVXC_Structure::ReadFAV(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
{
    std::string compression;
    ...
    pXML->FindElement("object");
    pXML->GetElAttribute("id", &ID);
    pXML->GetElAttribute("name", &Name);

    pXML->FindElement("grid");
    // read dimensions
    if(pXML->FindElement("dimension")){
        if (!pXML->FindLoadElement("x", &X_Voxels)) X_Voxels = 1;
        if (!pXML->FindLoadElement("y", &Y_Voxels)) Y_Voxels = 1;
        if (!pXML->FindLoadElement("z", &Z_Voxels)) Z_Voxels = 1;
        pXML->UpLevel();
    }else{
        // display error message
    }
    // read origin
    if(pXML->FindElement("origin")){
        pXML->FindLoadElement("x", &X_Origin);
        pXML->FindLoadElement("y", &Y_Origin);
        pXML->FindLoadElement("z", &Z_Origin);
        pXML->UpLevel();
    }else{
        // default value
        X_Origin = 0.0;
    ...
    if(pXML->FindElement("structure")){
        ...
        //get neighbors
        if(pXML->FindElement("link_map")){
            pXML->GetElAttribute("neighbors", &neighbors);
            Neighbors = std::stoi(neighbors);
            pXML->UpLevel();
        }else{
            Neighbors = 0;
        }

        CreateStructure(X_Voxels, Y_Voxels, Z_Voxels, Neighbors); //create the space for the structure

        if (Version == "" || Version == "0.1"){ //newest version

            if(pXML->FindElement("voxel_map")){
                int bit_per_voxel;
                pXML->GetElAttribute("compression", &compression);
                pXML->GetElAttribute("bit_per_voxel", &bit_per_voxel);
                ...
                std::cout << "error: no voxel data here" << std::endl;
            }
            pXML->UpLevel(); //Layer
            pXML->UpLevel(); //Data

            // Read color data
            if(pXML->FindElement("color_map")){

                has_color = true; //temporary
                pXML->GetElAttribute("compression", &compression);
                pXML->GetElAttribute("color_mode", &ColorMode);
                ...
                for (int i=0; i<Z_Voxels; i++){
                    ...
                    if (ColorMode == "RGB") {
                        ...
                        int cur = 0;
                        ColorIn_R = new float[RawData.size()/(2*3)]; // 1 voxel has 2*[Sample_Per_Voxel] bytes.
                        ColorIn_G = new float[RawData.size()/(2*3)];
                        ColorIn_B = new float[RawData.size()/(2*3)];

                        for (int i=0; i<(int)RawData.size(); i=i+6){
                            int i_r, i_g, i_b;
                            char r[2] = {RawData[i],RawData[i+1]};
                            char g[2] = {RawData[i+2],RawData[i+3]};
                            char b[2] = {RawData[i+4],RawData[i+5]};
                            sscanf(r, "%02x", &i_r);
                            sscanf(g, "%02x", &i_g);
                            sscanf(b, "%02x", &i_b);
                            ColorIn_R[cur] = i_r; //if compressed using this scheme
                            ColorIn_G[cur] = i_g; //if compressed using this scheme
                            ColorIn_B[cur] = i_b; //if compressed using this scheme
                            cur++;
                        }
                    }
                    ...
                    for(int y=0; y<Y_Voxels; y++){
                        for(int x=0; x<X_Voxels; x++){
                            ...
                            if(pData[index] != 0){
                                //the object's internal representation at this stage is as a long array, starting at (x0,xy,z0), proceeding to (xn,y0,z0), next to (xn,yn,z0), and on to (xn,yn,zn)
                                SetColorR(index, ColorIn_R[cur]);
                                SetColorG(index, ColorIn_G[cur]);
                                SetColorB(index, ColorIn_B[cur]);
                                cur++;
                            }
                        }
...
//                        if(pXML->FindLoadElement("layer", &RawData, true, true)){
//                            hasLayerData = true;
//                            std::vector<std::vector<int>> LinkMapIn = std::vector<std::vector<int>>(RawData.size()/(2*neighbors),std::vector<int>(Neighbors,0));
//                            int current_cell = 0;
//                            int current_neighbour = 0;

//                            for (int i=0; i<(int)RawData.size(); i=i+2){
//                                int bond_value_i;
//                                char bond_value_c[2] = {RawData[i],RawData[i+1]};
//                                sscanf(bond_value_c, "%02x", &bond_value_i);

//                                LinkMapIn[current_cell][current_neighbour] = bond_value_i;

//                                if(current_neighbour> 0 && current_neighbour%2==0) {
//                                    current_cell++;
//                                    current_neighbour = 0;
//                                }else{
//                                    current_neighbour++;
...
//                                            LinkMap[index][i] = LinkMapIn[current_link][i];
//                                        }
//                                        current_link++;
//                                    }
//                                }
...
void CVXC_Structure::CreateStructure(int xV, int yV, int zV, int nb) //creates empty structure with these dimensions
{
    IniData(xV*yV*zV, nb);
...
////////////////////////////////VOXEL INFO////////////////////////////////
CVXC_VoxelInfo::CVXC_VoxelInfo(std::string newName, int id, float r, float g, float b, float a, int geo_id, int mat_id)
{
    Name = newName;
    Id = id;
    SetColor(r,g,b,a);
    geo_id = NULL;
    Material_Info tmpMat;
    tmpMat.id = mat_id;
    tmpMat.ratio = NULL;
    Material_List.push_back(tmpMat);
}
void CVXC_VoxelInfo::ReadFAV(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
{
    pXML->GetElAttribute("id", &Id);
    pXML->GetElAttribute("name", &Name);

    pXML->FindElement("geometry_info");
    pXML->FindLoadElement("id",&Geometry_Id);
    pXML->UpLevel();


    Material_List.clear();
    while(pXML->FindElement("material_info")){
        Material_Info tmpMI;
        pXML->FindLoadElement("id",&tmpMI.id);
        pXML->FindLoadElement("ratio",&tmpMI.ratio);
        Material_List.push_back(tmpMI);
    }
    Material_Num = Material_List.size();

    if(pXML->FindElement("display")){
        pXML->FindLoadElement("r",&Red);
        pXML->FindLoadElement("g",&Green);
        pXML->FindLoadElement("b",&Blue);
        pXML->FindLoadElement("a",&Alpha);
        Red = Red / 255.0;
        Green = Green / 255.0;
        Blue = Blue / 255.0;
        Alpha = Alpha / 255.0;
...
void CVXC_VoxelInfo::WriteFAV(CXML_Rip* pXML, int Compression)
{
    pXML->SetElAttribute("id", GetId());
    if(GetName().size()>0) pXML->SetElAttribute("name", GetName());

    pXML->DownLevel("geometry_info");
    pXML->Element("id", GetGeometryId());
    pXML->UpLevel();

    for (int i=0; i<(int)Material_List.size(); i++){
        pXML->DownLevel("material_info");
        pXML->Element("id", Material_List[i].id);
        pXML->Element("ratio", Material_List[i].ratio);
        pXML->UpLevel();
    }

    pXML->DownLevel("display");
    pXML->Element("r", GetRedi());
    pXML->Element("g", GetGreeni());
    pXML->Element("b", GetBluei());
    pXML->Element("a", GetAlphai());
    pXML->UpLevel();

}

void CVXC_VoxelInfo::AddMatInfo(int id, double ratio)
{
    Material_Info tmp;
    tmp.id = id;
    tmp.ratio = ratio;
    Material_List.push_back(tmp);
}

void CVXC_VoxelInfo::SetDefaultColor(){
    switch(Id){
    case 1:
        SetColor(1,0,0,1.0);
        break;
...
CVXC_Geometry::CVXC_Geometry(int id, int shape_id, double scale_x, double scale_y, double scale_z)
{
    Id = id;
    ShapeId = shape_id;
    Scale_X = scale_x;
...
void CVXC_Geometry::ReadFAV(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
{

    pXML->GetElAttribute("id", &Id);
    pXML->GetElAttribute("name", &Name);

    if(pXML->FindLoadElement("shape", &Shape)){
        if(Shape == "cube") ShapeId = 0;
        else if(Shape == "sphere") ShapeId = 1;
    }

    pXML->FindElement("scale");
    pXML->FindLoadElement("x", &Scale_X);
    pXML->FindLoadElement("y", &Scale_Y);
    pXML->FindLoadElement("z", &Scale_Z);
}

void CVXC_Geometry::WriteFAV(CXML_Rip* pXML, int Compression)
{
    switch(ShapeId){
    case 0:
        Shape = "cube";
    ...
    pXML->Element("shape", Shape);
    pXML->DownLevel("scale");
    pXML->Element("x", Scale_X);
    pXML->Element("y", Scale_Y);
    pXML->Element("z", Scale_Z);
    pXML->UpLevel();
}

////////////////////////////////MATERIAL INFO////////////////////////////////

void CVXC_MaterialInfo::ReadFAV(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
{
    pXML->GetElAttribute("id",&Id);
    pXML->GetElAttribute("name",&Name);
    pXML->FindLoadElement("material_name", &Material_Name);

    if(pXML->FindElement("iso_standard")){
        pXML->FindLoadElement("iso_id", &Iso_Id);
        pXML->FindLoadElement("iso_name", &Iso_Name);
        pXML->UpLevel();
    }

    if(pXML->FindElement("product_info")){
        pXML->FindLoadElement("manufacturer", &Manufacturer, false, true);
        pXML->FindLoadElement("product_name", &Product_Name, false, true);
        pXML->FindLoadElement("url", &Product_Url, false, true);
        pXML->UpLevel();
    }
}

void CVXC_MaterialInfo::WriteFAV(CXML_Rip* pXML, int Compression)
{
    if(Material_Name.size()>0) pXML->Element("material_name", Name);
    if(Iso_Id.size()>0 || Iso_Name.size()>0) {
        pXML->DownLevel("iso_standard");
        pXML->Element("iso_id", Iso_Id);
        pXML->Element("iso_name", Iso_Name);
        pXML->UpLevel();
    }
    if(Manufacturer.size()>0 || Product_Name.size()>0 || Product_Url.size()>0){
        pXML->DownLevel("Product_info");
        pXML->Element("manufacturer", Manufacturer);
        pXML->Element("product_name", Product_Name);
        pXML->Element("url", Product_Url);
        pXML->UpLevel();
    }
}
