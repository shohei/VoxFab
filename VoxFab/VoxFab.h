...
class VoxFab : public QMainWindow
{
	Q_OBJECT

public:
    VoxFab(QWidget *parent = 0, QFlag::QFlag flags = 0);
    ~VoxFab();
...
//	CMesh MainMesh; //current best surface mesh approximation of our voxel object...
    ...
    //Material Palette Dialog:
    void SetupMatPaletteWindow(void);
    QDockWidget *MatPaletteDockWidget;
    Dlg_MatPalette *MatPaletteDlg;
    ...
    //Mesh Info Dialog
//    void SetupMeshInfoWindow(void);
//    QDockWidget *MeshInfoDockWidget;
//    Dlg_MeshInfo *MeshInfoDlg;
...
public slots: //global slot repository for updating things across all windows/views...
    ...
    void ExportFAV(){ MainObj.SaveZLib_FAV(); }
    void ImportSTL(){
        ForceViewMode();
        MainObj.New();
        QString NewFileName;
        if (!MainObj.ImportSTL(&NewFileName)) {
            return;
        }
        FastModeChanged(false);
        CheckNumVox();
        ZoomExtAll();
        SetWindowName(NewFileName);
        UpdateAllWins();
        WorkspaceDlg->UpdateUI();
    }
    ...
    void ImportFAV(){
        ...
        MainObj.New();
        QString NewFileName;
        if (!MainObj.ImportFAV(&NewFileName)) {
            return;
        }
        FastModeChanged(false);
        CheckNumVox();
    ...
    void EnableGraphics(bool Enabled){GraphicsEnabled = Enabled; ReqGLUpdateAll(); MainObj.DrawMode = DisplayMode::FullColor;}
	void CheckNumVox(void) {GraphicsEnabled = true; if(MainObj.GetNumVox() >= 1000000) {GraphicsEnabled = false; if (QMessageBox::question(NULL, "Disable Graphics?", "The selected file has greater than 1,000,000 voxels. Do you want to disable graphics?", QMessageBox::Yes | QMessageBox::No)!=QMessageBox::Yes) GraphicsEnabled = true; } ui.actionGraphics_Enabled->setChecked(GraphicsEnabled); }
        ...
        // color mode
    void SetColorView(void) {MainObj.DrawMode = DisplayMode::FullColor; ReqGLUpdateAll();}
    void SetMaterialView(void) {MainObj.DrawMode = DisplayMode::MatColor; ReqGLUpdateAll();}

    void UpdateAllWins() {EnterVMMode(VM_3DVIEW); PaletteDlg->UpdateUI(); WorkspaceDlg->UpdateUI(); VoxInfoDlg->UpdateUI(); ReqGLUpdateAll();}
    ...
    void ExportSTL_smooth(){if(MainObj.is_draw_smooth) MainObj.ExportSmoothSTL();}
...
//    void ViewMeshInfoWindow(bool Visible){if (Visible){MeshInfoDockWidget->show();MeshInfoDockWidget->raise();ui.actionInfo->setChecked(true);} else {MeshInfoDockWidget->hide();ui.actionInfo->setChecked(false);}}
...
private:
    ...
    void SetWindowName(QString Filename){setWindowTitle(Filename + " - VoxFab");}
};
